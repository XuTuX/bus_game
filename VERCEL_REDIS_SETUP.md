...
비밀값은 문서에 적지 않습니다. `REDIS_URL` 같은 Redis 접속 값은 Vercel 환경변수와 로컬 `.env.*.local` 파일에만 둡니다.
...
현재 앱은 방 상태를 Redis에 저장합니다.

1. 기본값: `REDIS_URL` 사용
2. 또는 `UPSTASH_REDIS_REST_URL`과 `UPSTASH_REDIS_REST_TOKEN` 사용
3. 또는 같은 REST 값의 alias로 `KV_REST_API_URL`과 `KV_REST_API_TOKEN` 사용

Redis 환경변수가 없으면 앱은 방 상태를 저장할 수 없습니다.
...
Next.js 서버 코드에서 `REDIS_URL`이 있으면 Redis 프로토콜로 접속합니다.

REST 환경변수 조합이 있으면 기존처럼 Redis REST API를 호출합니다.
...
Upstash REST 변수도 대체 방식으로 지원합니다.
...
정상이라면 다음처럼 Redis 환경변수가 보여야 합니다.
...
REST 값이 없고 `REDIS_URL`이 있으면 Redis 프로토콜로 직접 접속합니다.
...
### `.env.development.local`에 Redis 환경변수가 없는 경우

Redis 환경변수가 `Preview, Production`에만 있고 `Development`에는 없을 수 있습니다.
...
### 로컬에서 Redis 환경변수가 없는 경우

로컬 `.env.local`이나 `.env.development.local`에 Redis 환경변수가 없으면 방 생성과 상태 저장 요청이 실패합니다.

로컬에서도 Vercel에서 받은 `REDIS_URL` 값을 설정해야 합니다.
